...
function zoneOptions(current: string): string[] {
  ...
  for (const p of PRESET_PLACES) merged.add(p.tz);
  return [...merged].sort();
}
...
export default function BirthSkyPanel() {
  ...
  const caption: SkyCaption = useMemo(() => {
    ...
    return {
      title: name.trim() ? `The sky when ${name.trim()} was born` : "Your sky",
  ...
  function useMyLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocateError("Couldn't get your location — enter coordinates instead.");
        setLocating(false);
      },
      { timeout: 10000 }
    );
  ...
  function savePng() {
    ...
    canvas.toBlob((blob) => {
      ...
      const slug = (name.trim() || "sky").toLowerCase().replace(/[^a-z0-9]+/g, "-");
      a.href = url;
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-white/10 bg-black/40 p-4 backdrop-blur-md">
      <div>
        <h2 className="text-sm font-medium text-neutral-200">Your Sky</h2>
        <p className="mt-1 text-xs text-neutral-500">
          The real sky overhead at a moment and place — every star where it
          actually was, computed from catalogue positions and planetary
          ephemerides.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <label className="flex flex-col gap-1 text-xs text-neutral-300">
          Name <span className="text-neutral-600">(optional, for the caption)</span>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. Pranjal"
            className="rounded-md border border-white/10 bg-black/40 px-2 py-1 text-xs text-neutral-200 placeholder:text-neutral-600 focus:border-indigo-400 focus:outline-none"
          />
        </label>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="flex flex-col gap-1 text-xs text-neutral-300">
            ...
            <input
              ...
              className="rounded-md border border-white/10 bg-black/40 px-2 py-1 text-xs text-neutral-200 [color-scheme:dark] focus:border-indigo-400 focus:outline-none"
            />
          </label>
          <label className="flex flex-col gap-1 text-xs text-neutral-300">
            Local time
        ...
        <label className="flex flex-col gap-1 text-xs text-neutral-300">
          <span className="flex justify-between">
            <span>Timezone</span>
            {moment && (
              <span className="text-neutral-500">{zoneLabel(moment, tz)}</span>
            )}
          </span>
          <select
            value={tz}
            onChange={(e) => setTz(e.target.value)}
            className="rounded-md border border-white/10 bg-black/40 px-2 py-1 text-xs text-neutral-200 focus:border-indigo-400 focus:outline-none"
          >
            {zones.map((z) => (
        ...
        <div className="flex flex-col gap-1.5">
          <span className="text-xs text-neutral-300">Place</span>
          ...
          {locateError && (
            <p className="text-xs text-amber-400">{locateError}</p>
          )}
        </div>
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-300">
            ...
            <button
              onClick={savePng}
              disabled={saving}
              className="ml-auto rounded-md bg-indigo-500 px-3 py-1 text-white hover:bg-indigo-400 disabled:opacity-60"
            >
          ...
          <div className="flex flex-col gap-1.5 border-t border-white/10 pt-3 text-xs text-neutral-500">
            ...
            <p className="text-neutral-600">
              ...
              as if you were lying on your back looking up. The time is read as
              wall-clock time in {tz} ({zoneLabel(moment, tz)}), with historical
              daylight-saving rules applied. Star positions are J2000 catalogue
              coordinates, which drift under half a degree over a lifetime.
            </p>
          </div>
        </>
      ) : (
        <p className="text-xs text-amber-400">
          That date and time couldn&apos;t be read — check the values above.
        </p>
      )}
    </div>
  );
